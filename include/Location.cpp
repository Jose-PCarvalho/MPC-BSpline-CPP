...
Line::Line(Location i, Location f)
{
    ...
    this->length = vec.norm();
    this->dist_thresh = 0.04;
...
Eigen::Vector2d Line::getSpeed(Location intr,double velocity,bool final)
{
    double dist2end = getDist2End(intr);
    Eigen::Vector2d v =velocity*this->getVec();
    if (final and length <=0.1)
        v = 0.7*v;
    if (dist2end<=2*dist_thresh and not final){
        v = 0.8*v;
    }
    if (dist2end<=3*dist_thresh and  final){
        v = 0.7*v;
    }
    if (dist2end<=2*dist_thresh and  final){
        v = 1*v;
    }
    if (dist2end >0.2)
    {
